...
class OrderController extends Controller
{
  ...
  public function index(Request $request)
  {
    try {
      ...
      // Hanya return JSON jika ini request AJAX DAN BUKAN request dari Inertia
      if (($request->wantsJson() || $request->header('X-Requested-With')) && !$request->header('X-Inertia')) {
        return response()->json([
          'orders' => $orders, // Cukup kirim orders saja untuk infinite scroll
        ]);
      }
